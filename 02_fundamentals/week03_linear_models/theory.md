...
Weeks 01–02 developed the **optimisation machinery** — gradient descent, momentum, Adam — as general-purpose tools for minimising any differentiable function. This week we apply those tools (and a powerful alternative: the closed-form normal equations) to the two most fundamental supervised learning models:
...
**Prerequisites.** Week 00b (linear algebra, calculus, probability basics), Weeks 01–02 (gradient descent, optimisers, learning rate schedules).

---

## 2. The Supervised Learning Setup

Recall from Week 00a:
...
Unlike the general optimisation problems in Weeks 01–02, linear regression has a **closed-form solution**: we can compute $\mathbf{w}^*$ directly, without iterating.
...
Take the gradient with respect to $\mathbf{w}$ and set it to zero. Recall the matrix calculus identities from Week 00b:
...
**When $X^\top X$ is singular.** If $n < d + 1$ (more parameters than data points) or if features are collinear, $X^\top X$ is singular and the normal equations have infinitely many solutions. This is where **regularisation** (Week 04) becomes essential: Ridge regression adds $\lambda I$ to $X^\top X$, guaranteeing invertibility.
...
**Convergence.** Since the MSE loss for linear regression is a convex quadratic, the convergence analysis from Week 01 applies directly. The Hessian is:
...
And the convergence rate per step is $\rho = \frac{\kappa - 1}{\kappa + 1}$ (from Week 01, Section 4.3).
...
> **Why this matters for the course.** From Week 04 onwards, every model uses gradient-based optimisation. The linear regression closed-form solution is the **only** case in the course where the exact answer can be computed without iteration. Appreciate this simplicity — it never returns.
...
> **Key insight.** Minimising MSE is equivalent to maximum likelihood estimation under the assumption of Gaussian noise. The $\sigma^2$ term disappears from the optimisation because it does not depend on $\mathbf{w}$. This connection will be deepened in Week 07 (Likelihood).
...
> **What this means.** If you restrict yourself to (i) linear functions of $\mathbf{y}$ that (ii) are unbiased, OLS is the best you can do. This is a strong result, but note the caveats: biased estimators (like Ridge regression, Week 04) can have **lower MSE** by trading bias for a large reduction in variance.
...
>
> **Forward pointer.** Softmax regression is the output layer of every classification neural network (Weeks 11–12). Understanding it here means you already understand how neural network classifiers make predictions.
...
The main approaches (detailed in Week 04/06):
...
| Week                          | Connection                                                                                                                        |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Week 04 (Regularisation)**  | Ridge and Lasso add L2/L1 penalties to the models built here                                                                      |
| **Week 07 (Likelihood)**      | The MLE derivation of Section 4.2 is generalised to arbitrary distributions                                                       |
| **Week 08 (Uncertainty)**     | Bayesian linear regression puts a prior on $\mathbf{w}$ — Ridge regression is the MAP estimate with a Gaussian prior              |
| **Week 11 (NN from scratch)** | A single-layer neural network without activation is exactly linear regression; with sigmoid activation, it is logistic regression |
| **Week 13 (PyTorch)**         | `nn.Linear(d, 1)` implements $\mathbf{w}^\top\mathbf{x} + b$                                                                      |
| **Week 18 (Transformers)**    | The attention mechanism outputs a weighted sum — a *learned* linear regression                                                    |
