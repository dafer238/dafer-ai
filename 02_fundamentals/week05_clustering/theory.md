...
All models in Weeks 01–04 were **supervised**: they learned from labelled data $\{(\mathbf{x}_i, y_i)\}$. This week introduces **unsupervised learning**: discovering structure in unlabelled data $\{\mathbf{x}_i\}$.
...
- **Latent-space analysis** — understanding what deep models learn (Weeks 11, 15).
...
**Prerequisites.** Week 00b (distance metrics, linear algebra), Week 04 (PCA for visualisation of clusters in 2D).
...
> ```
> This is the same issue as PCA (Week 04, Section 8.1).
...
- **Update step:** setting $\boldsymbol{\mu}_k$ to the mean of $C_k$ minimises $\sum_{\mathbf{x}_i \in C_k}\|\mathbf{x}_i - \boldsymbol{\mu}_k\|^2$ (the mean is the least-squares optimal centre — this is the same argument as the normal equations in Week 03, but for a single cluster).
...
| Identifies noise/outliers           | Degrades in high dimensions (distances concentrate, Section 2 of Week 04)                                     |
...
> **Forward pointer.** The EM algorithm is the topic of Week 07 (Likelihood) and Week 08 (Uncertainty), where it is derived rigorously from the perspective of maximum likelihood estimation with latent variables.
...
| High-dimensional data                         | **PCA + K-means** or **spectral clustering** | Reduce dimesionality first (Week 04)    |
...
| Week                         | Connection                                                                                                               |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Week 04 (PCA)**            | PCA projections used to visualise clusters in 2D; PCA whitening improves K-means (Section 7 of Week 04)                  |
| **Week 06 (Regularisation)** | Regularised GMMs (adding a small diagonal to $\Sigma_k$) prevent singular covariance — same idea as Ridge regularisation |
| **Week 07 (Likelihood)**     | GMM fitting is MLE with latent variables — the EM algorithm is derived formally                                          |
| **Week 08 (Uncertainty)**    | Bayesian GMMs with Dirichlet priors; variational inference as generalised EM                                             |
| **Week 11 (NNs)**            | Neural network latent representations can be clustered to understand what the network has learned                        |
| **Week 15 (CNNs)**           | Feature map activations clustered to discover semantic concepts                                                          |
| **Week 18 (Transformers)**   | Attention heads exhibit cluster-like specialisation across sequence positions                                            |
