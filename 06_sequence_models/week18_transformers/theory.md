...
This week assembles components from Week 17 (attention) and earlier weeks (residual connections, layer normalisation, dropout) into the **Transformer** — the architecture behind BERT, GPT, T5, Vision Transformers, and virtually every modern foundation model.
...
**Prerequisites.** Week 17 (scaled dot-product attention, multi-head attention, masking). Week 12 (residual connections, LayerNorm, BatchNorm). Week 13–14 (PyTorch `nn.Module`, training loops).
...
Exactly the multi-head attention from Week 17, applied as **self-attention** ($Q = K = V = x$):
...
This is identical to the residual connections from Week 12 (ResNets). Benefits:
...
**LayerNorm vs. BatchNorm** (from Week 12):
...
| Week | Connection |
|---|---|
| **Week 11 (NN from Scratch)** | Backpropagation through the computational graph still applies — the chain rule handles attention→softmax→matmul |
| **Week 12 (Training Pathologies)** | Residual connections and LayerNorm are essential Transformer components; originated from "vanishing gradient" solutions |
| **Week 13 (PyTorch)** | `nn.Module` pattern, `state_dict`, training loop — all reused for Transformer training |
| **Week 14 (Training at Scale)** | Warm-up schedules, mixed precision (FP16 attention scores), gradient accumulation for large batch sizes |
| **Week 15 (CNNs)** | FFN is like a 1×1 convolution; Transformers in vision (ViT) replace convolutions entirely |
| **Week 16 (Regularisation)** | Dropout at five locations; weight decay with AdamW; data augmentation for Transformer pre-training (masking, span corruption) |
| **Week 17 (Attention)** | Scaled dot-product attention, multi-head attention, masking — all building blocks assembled here |
| **Week 19 (Fine-Tuning)** | Pretrained Transformer weights from HuggingFace — knowing the architecture lets you fine-tune intelligently |
...
| Remove residual connections from all layers | Training collapses for $N > 2$; gradients vanish just as they did in deep MLPs (Week 12) |
