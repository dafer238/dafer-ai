...
This week introduces **attention mechanisms** — the core operation that replaced recurrence in modern sequence models and enabled Transformers. Implementing attention from scratch this week means that Week 18 (Transformers) is assembling known components, not absorbing an alien architecture.
...
**Prerequisites.** Week 13 (PyTorch `nn.Module`, training loop). The concept of "sequential data" from Week 00a.
...
**Critical limitation.** RNNs are sequential by construction — $h_t$ depends on $h_{t-1}$. This prevents parallelisation and, despite gating, gradients still degrade over long sequences (connections to Week 12: vanishing gradients).
...
For large $d_k$, the scores have large magnitudes, pushing softmax into saturation (outputting near-zero or near-one). In saturation, gradients vanish (same problem as sigmoid, Week 12).
...
A Transformer (Week 18) stacks attention with two more components:
...
- **Residual connections** (Week 12) — $x + \text{Attention}(x)$.
- **Layer normalisation** (Week 12) — normalise across features, not batch.
...
Week 18 assembles these into the full architecture.
...
| Week                               | Connection                                                                                                                                      |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Week 12 (Training Pathologies)** | Residual connections and LayerNorm are essential in Transformers; vanishing gradients in RNNs motivate the move to attention                    |
| **Week 13 (PyTorch)**              | Attention implemented as `nn.Module`; same training loop, `state_dict`, checkpointing                                                           |
| **Week 14 (Training at Scale)**    | Long sequences → large memory; mixed precision essential; gradient clipping common                                                              |
| **Week 15 (CNNs)**                 | Self-attention can be seen as a data-dependent convolution with a global receptive field — each "filter" is determined by content, not position |
| **Week 16 (Regularisation)**       | Dropout applied after attention weights and after FFN; weight decay with AdamW standard for attention models                                    |
| **Week 18 (Transformers)**         | Assembles multi-head attention into a full encoder–decoder architecture with positional encoding                                                |
| **Week 19 (Fine-Tuning)**          | Attention weights in pretrained models encode learned relationships; fine-tuning adapts them to new tasks                                       |
...
| Measure wall-clock time for self-attention with $T \in \{64, 128, 256, 512, 1024\}$     | Quadratic scaling empirically visible; motivates efficient attention in Week 18+            |
