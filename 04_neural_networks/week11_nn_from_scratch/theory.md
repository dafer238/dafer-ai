...
Weeks 01–10 built models from the probabilistic toolkit: linear regression, regularisation, MLE, Bayesian inference, GPs. All of these are powerful but fundamentally limited in the functions they can represent (linear in features, or requiring hand-crafted kernels).
...
After this week, when you use `loss.backward()` in PyTorch (Week 13), you will know exactly what it computes.

**Prerequisites.** Week 00b (chain rule), Weeks 01–02 (gradient descent, optimisers), Week 03 (linear regression as a single neuron).
...
This is exactly logistic regression (Week 03) when $\sigma = \text{sigmoid}$ and the loss is binary cross-entropy — so a **logistic regression unit is a single neuron**.
...
**Connection to likelihood (Week 07):** cross-entropy loss = negative log-likelihood of the Bernoulli (BCE) or Categorical (CE) distribution. Minimising CE = maximising likelihood.
...
where $Y$ is the one-hot encoded label matrix. This is the same as the logistic regression gradient (Week 03) — no coincidence; the derivation is identical.
...
The backward pass traverses this graph in **reverse topological order**, applying the chain rule at each node. This is exactly what PyTorch's `autograd` does automatically (Week 13).
...
For the `make_moons` dataset ($m = 210$ training samples), this is fine. For larger datasets, use **mini-batch SGD** (Weeks 01–02):
...
- Acts as implicit regularisation (Week 06).
...
**Connection to Bayesian inference (Week 08):** L2 regularisation = Gaussian prior on weights with precision $\alpha = 2\lambda/\sigma_n^2$. The regularised loss = negative log-posterior (MAP estimate).

**Other forms** (covered in later weeks):
- **Dropout** (Week 16) — randomly zero out neurons during training.
- **Early stopping** (Week 06) — stop training when validation loss increases.
- **Batch normalisation** (Week 12) — normalises activations; has implicit regularising effect.
- **Data augmentation** (Week 15) — artificially increase training set.
...
| Week                               | Connection                                                                                                         |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **Week 01–02 (Optimisation)**      | Gradient descent and advanced optimisers (Adam, momentum) are used unchanged; only the gradient computation is new |
| **Week 03 (Linear Models)**        | A single neuron with sigmoid = logistic regression; a single neuron with identity = linear regression              |
| **Week 06 (Regularisation)**       | L2 regularisation = weight decay; early stopping applies directly                                                  |
| **Week 07 (Likelihood)**           | Cross-entropy loss = NLL; MSE loss = Gaussian NLL                                                                  |
| **Week 08 (Uncertainty)**          | Weight decay = Gaussian prior (MAP); Bayesian NNs place a full posterior on $W$                                    |
| **Week 10 (GPs)**                  | Infinite-width single-hidden-layer network → GP (Neal, 1996)                                                       |
| **Week 12 (Training Pathologies)** | Vanishing/exploding gradients; batch norm; residual connections                                                    |
| **Week 13 (PyTorch)**              | PyTorch autograd computes exactly what `backward()` computes here, but automatically                               |
| **Week 14 (Training at Scale)**    | Mini-batch SGD, learning rate schedules, distributed training                                                      |
| **Week 15 (CNNs)**                 | Convolutions are a specialised layer type; backprop extends naturally                                              |
